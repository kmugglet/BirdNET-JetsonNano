...
if ! grep PRIVACY_THRESHOLD /etc/birdnet/birdnet.conf &>/dev/null;then
  ...
  git -C $HOME/BirdNET-JetsonNano rm $my_dir/privacy_server.py
fi
...
if ! grep 'BirdNET-JetsonNano/birdnet/' $HOME/BirdNET-JetsonNano/templates/birdnet_server.service &>/dev/null || ! grep 'BirdNET-JetsonNano/birdnet' $HOME/BirdNET-JetsonNano/templates/chart_viewer.service &>/dev/null;then
  sudo -E sed -i "s|ExecStart=.*|ExecStart=$HOME/BirdNET-JetsonNano/birdnet/bin/python3 /usr/local/bin/server.py|" ~/BirdNET-JetsonNano/templates/birdnet_server.service
  sudo -E sed -i "s|ExecStart=.*|ExecStart=$HOME/BirdNET-JetsonNano/birdnet/bin/python3 /usr/local/bin/daily_plot.py|" ~/BirdNET-JetsonNano/templates/chart_viewer.service
  sudo systemctl daemon-reload && restart_services.sh
fi

if grep privacy ~/BirdNET-JetsonNano/templates/birdnet_server.service &>/dev/null;then
  sudo -E sed -i 's/privacy_server.py/server.py/g' \
    ~/BirdNET-JetsonNano/templates/birdnet_server.service
  sudo systemctl daemon-reload
  restart_services.sh
fi
if ! grep APPRISE_NOTIFICATION_TITLE /etc/birdnet/birdnet.conf &>/dev/null;then
  sudo -u$USER echo "APPRISE_NOTIFICATION_TITLE=\"New BirdNET-JetsonNano Detection\"" >> /etc/birdnet/birdnet.conf
fi
...
apprise_installation_status=$(~/BirdNET-JetsonNano/birdnet/bin/python3 -c 'import pkgutil; print("installed" if pkgutil.find_loader("apprise") else "not installed")')
if [[ "$apprise_installation_status" = "not installed" ]];then
  $HOME/BirdNET-JetsonNano/birdnet/bin/pip3 install -U pip
  $HOME/BirdNET-JetsonNano/birdnet/bin/pip3 install apprise
fi
[ -f $HOME/BirdNET-JetsonNano/apprise.txt ] || sudo -E -ucaddy touch $HOME/BirdNET-JetsonNano/apprise.txt
...
if grep bash $HOME/BirdNET-JetsonNano/templates/web_terminal.service &>/dev/null;then
  sudo sed -i '/User/d;s/bash/login/g' $HOME/BirdNET-JetsonNano/templates/web_terminal.service
  sudo systemctl daemon-reload
  sudo systemctl restart web_terminal.service
fi
[ -L ~/BirdSongs/Extracted/static ] || ln -sf ~/BirdNET-JetsonNano/homepage/static ~/BirdSongs/Extracted
...
if systemctl list-unit-files pushed_notifications.service &>/dev/null;then
  ...
  sudo rm $HOME/BirdNET-JetsonNano/templates/pushed_notifications.service
fi

if [ ! -f $HOME/BirdNET-JetsonNano/model/labels.txt ];then
...
pytest_installation_status=$(~/BirdNET-JetsonNano/birdnet/bin/python3 -c 'import pkgutil; print("installed" if pkgutil.find_loader("pytest") else "not installed")')
if [[ "$pytest_installation_status" = "not installed" ]];then
  $HOME/BirdNET-JetsonNano/birdnet/bin/pip3 install -U pip
  $HOME/BirdNET-JetsonNano/birdnet/bin/pip3 install pytest==7.1.2 pytest-mock==3.7.0
fi

[ -L ~/BirdSongs/Extracted/weekly_report.php ] || ln -sf ~/BirdNET-JetsonNano/scripts/weekly_report.php ~/BirdSongs/Extracted

if ! grep weekly_report /etc/crontab &>/dev/null;then
  sed "s/\$USER/$USER/g" $HOME/BirdNET-JetsonNano/templates/weekly_report.cron | sudo tee -a /etc/crontab
fi
...
if ! grep '\-\-browser.gatherUsageStats false' $HOME/BirdNET-JetsonNano/templates/birdnet_stats.service &>/dev/null ;then
  sudo -E sed -i "s|ExecStart=.*|ExecStart=$HOME/BirdNET-JetsonNano/birdnet/bin/streamlit run $HOME/BirdNET-JetsonNano/scripts/plotly_streamlit.py --browser.gatherUsageStats false --server.address localhost --server.baseUrlPath \"/stats\"|" $HOME/BirdNET-JetsonNano/templates/birdnet_stats.service
  sudo systemctl daemon-reload && restart_services.sh
fi
