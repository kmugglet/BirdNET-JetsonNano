...
my_dir=$HOME/BirdNET-JetsonNano
export my_dir=$my_dir

cd $my_dir/scripts || exit 1

if [ "$(uname -m)" != "aarch64" ];then
  echo "BirdNET-JetsonNano requires a 64-bit OS.
...
install_birdnet() {
  cd ~/BirdNET-JetsonNano || exit 1
  echo "Establishing a python virtual environment"
  python3 -m venv birdnet
  source ./birdnet/bin/activate
  pip3 install -U -r $HOME/BirdNET-JetsonNano/requirements.txt
}
